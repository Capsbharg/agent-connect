...
- **Agents run headless.** `ClaudeAgent` passes `--dangerously-skip-permissions` and `CursorAgent` passes `--force` by default (`CLAUDE_SKIP_PERMISSIONS` / `CURSOR_FORCE`) — this is required for a CLI agent to respond to a chat message without a human approving every action in a terminal. The actual access boundary is `ALLOWED_USERS`/`ALLOWED_CHANNELS`/`ALLOWED_GROUPS`, not the agent CLI's own confirmation prompts.
