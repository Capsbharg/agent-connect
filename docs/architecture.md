...
- `ClaudeAgent`/`CursorAgent`/`GeminiAgent` run headless by default (`--dangerously-skip-permissions` / `--force` / `--yolo`, gated by `CLAUDE_SKIP_PERMISSIONS`/`CURSOR_FORCE`/`GEMINI_YOLO`) — no per-action confirmation, so `AllowListAuthorizationProvider` is the actual access boundary, not the agent CLI itself. `GeminiAgent` additionally always passes `--skip-trust`, unconditionally (no config toggle) — without it, Gemini refuses to run at all in a project directory that hasn't already been manually trusted via an interactive session, which every project reached through this framework's headless pipeline would otherwise hit. `AgentConnect.start()` logs a loud warning when `ALLOWED_USERS`/`ALLOWED_CHANNELS`/`ALLOWED_GROUPS` are all empty.
...
- Multi-turn session continuation (`ClaudeAgent`'s `--resume`, `CodexAgent`'s `exec resume`, `GeminiAgent`'s `--resume`) is wired up for the three CLIs whose resume semantics were verified against a real install to take a raw session id. Gemini's is a special case worth calling out: `gemini --help` documents `--resume` as index/"latest"-based only, but a live install confirmed it also accepts a raw session UUID and correctly restores prior context — the CLI's own error text on an invalid id even lists `--resume {uuid}` as valid usage, so this is undocumented-but-sanctioned behavior, not an accident being relied on. `CursorAgent` leaves it unwired — Cursor's CLI ships no native Windows build (confirmed: its installer only targets Linux/Darwin, and the download host 403s for a `windows`/`win32` target), so `--resume`'s semantics couldn't be verified there; wire it the same way once verified on a supported platform.
