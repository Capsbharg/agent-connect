...
**Multi-turn conversations.** With Claude or Codex, your next prompt in the same project automatically continues the previous conversation (Claude's `--resume`, Codex's `exec resume`) instead of starting fresh each time — the agent keeps whatever context it built up. Run `use <project>` again (even re-selecting the project you're already in) to intentionally start a new conversation. If a stored conversation ever becomes stale (expired, pruned by the CLI, lost across an upgrade), the next prompt detects the failed resume and automatically starts a fresh conversation instead of retrying the same broken one forever. Cursor and Gemini don't support this yet — see [docs/architecture.md](docs/architecture.md) for why.
